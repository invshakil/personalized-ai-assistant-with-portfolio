...
| `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` | OAuth client shared by **DB backups** (Drive API, redirect `…/api/admin/backup/google/callback`) **and Booking** (Calendar API, redirect `…/api/admin/booking/google/callback`). One client, both APIs + both redirect URIs. Optional — omit to disable Drive upload + online booking |
| `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY`           | Cloudflare Turnstile keys protecting the **public** booking form (`TURNSTILE_SECRET` accepted as a fallback name). Optional — if unset, the captcha is skipped (honeypot + per-IP rate limit still apply)                                                                             |
