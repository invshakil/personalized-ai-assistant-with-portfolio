...
| `Voucher`                                 | Credit applied against ONE month's bill — a discount, or reimbursing a cost the tenant fronted that the landlord owes (e.g. maintenance). Mirror of `OneOffCharge`: stored POSITIVE, SUBTRACTED from `rentDue`; `label`, `amount`, `month`, `year`; linked to a `Payment` by `(tenantId, month, year)`. Capped at the outstanding bill so add/edit/delete stay an exact round-trip (an uncapped voucher would hit the `max(0, …)` floor and over-restore on delete). Excess credit is NOT carried to the next month                                                                                                                                                                           |
...
| `Trip`                                    | Trip Expense Manager — a trip (group-capable); `destination`, `localCurrency` (e.g. MYR), `homeCurrency`, `startDate`/`endDate?`, `status` (PLANNING/ACTIVE/CLOSED), `localWalletAccountId?` (foreign wallet `MoneyAccount`), `publicSlug?`/`isPublic`/`publicIntro?` (shareable `/trips/<slug>` page). Group cost derived from `TripExpense` rows                                                                                                                                                                                                                                                                                                                                            |
| `TripBudget`                              | Trip Expense Manager — one planned budget per `TripCategory` (FLIGHTS/ACCOMMODATION/FOOD/LOCAL_TRANSPORT/ACTIVITIES/SHOPPING/VISA_INSURANCE/MISC); `plannedAmount` (BDT). Unique `[tripId, category]`, cascade-deleted with the trip                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `TripParticipant`                         | Trip Expense Manager (v2) — a person on a trip; `name`, `isSelf` (exactly one "Me" per trip), optional `beneficiaryId` link, `isActive` (soft-delete keeps split history). Cascade-deleted with the trip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `TripExpense`                             | Trip Expense Manager (v2) — the source of truth for a shared cost; `category`, `amount`/`currency`/`fxRate`/`amountBdt`, `payerId`, `splitMode` (EQUAL/EXACT), optional `accountId` (funding account) + `moneyEntryId?` (unique; set only when the posting rule fires: self + real non-card account). Has many `TripExpenseShare`                                                                                                                                                                                                                                                                                                                                                             |
| `TripExpenseShare`                        | Trip Expense Manager (v2) — one participant's slice of a `TripExpense`; `amount` (expense currency) + `amountBdt`. Unique `[expenseId, participantId]`; shares sum exactly to the expense                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `TripSettlement`                          | Trip Expense Manager (v2) — money handed between two participants (collect fund / settle up); `fromParticipantId`→`toParticipantId`, `amount`/`currency`/`fxRate`/`amountBdt`. Pure trip ledger — never posts a `MoneyEntry`                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `BookingSettings`                         | Booking (consultation scheduler) — singleton (id `"singleton"`): `enabled`, `calendarId`, meeting title template, `durationsMinutes[]`, slot increment/buffer/min-notice/max-horizon, `timezone`, `workingHours` JSON (`[{weekday,start,end}]`), WhatsApp block, `googleConnected`/`googleEmail` + AES-256-GCM-encrypted `googleToken*` triple (refresh token)                                                                                                                                                                                                                                                                                                                                |
| `BookingBlackout`                         | Booking — a closure day (holiday/vacation) removed from availability; `@@unique([date])`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `Booking`                                 | Booking — one visitor booking; `name`/`email`/`topic`/`message`, `startsAt`/`endsAt`/`durationMin`, `status` (`BookingStatus` PENDING/CONFIRMED/CANCELLED), `googleEventId`/`meetUrl`, unguessable `cancelToken @unique` (self-cancel link), `ip` (rate-limit), cancel fields                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
│   │   │       ├── money/             ✅ complete — Money Manager: overview, ledger, people & loans, accounts, categories, import CSV
│   │   │       ├── trips/             ✅ complete — Trip Expense Manager (v2 group): list + [id] detail (participants, split, settlements, who-owes-whom, wallet, publish)
│   │   │       ├── bookings/          ✅ Booking list (visitor consultations); cancel action (deletes Google Calendar event)
│   │   │       ├── reports/           ✅ Reports hub — financial/ + property/ + solar/; page.tsx → /reports/financial
│   │   │       ├── renovation/        🔲 stub (MUI StubPage)
│   │   │       ├── ai-assistant/      ✅ MUI chat UI — streaming, provider-swappable, read + write (approval-gated) tool use, ConversationList
│   │   │       ├── settings/          ✅ Site Settings; ai (provider + budget); appearance (theme); backup; solar; booking (consultation scheduler config)
...
│   │   ├── ai/                    ✅ provider seam — registry, adapters/anthropic, config, crypto, tools, writeTools/, sessions, usage, pricing
│   │   ├── money/                 ✅ Money Manager — accounts, entries, people/loans, categories, import, dashboard
│   │   ├── trips/                 ✅ Trip Expense Manager — trips, participants, expenses, settlements, fund, report, public, _split
│   │   ├── booking/               ✅ Booking — settings, slots (availability math), bookings, googleCalendar (OAuth+Calendar REST)
│   │   ├── solar/                 ✅ Solar — tariff slab math, reports, weather (Open-Meteo), payback
│   │   ├── solis/                 ✅ SolisCloud signed READ-ONLY client + sync + scheduler + fieldMap
│   │   └── _shared/               ✅ dateRange (period→{from,to}) + fx (live BDT rate cache)
│   ├── store/                     ✅ Redux Toolkit — store, hooks, StoreProvider (in AdminShell), slices/ (transient client state only)
│   ├── hooks/                     ✅ genuinely-shared hooks — useConfirmDialog, useCurrencyOptions, useMoneyAccounts, useSpeechRecognition
│   ├── lib/
...
│   │   └── api/                   ✅ Axios client layer — client, admin, finance, property, money, trips, booking, currencies, ai
│   ├── middleware.ts               ✅ protects /admin/* routes
│   └── types/                     ✅ per-module types (admin, finance, property, money, trip, fx, solar, booking, portfolio) + index barrel
...
### Trip Expense Manager (`/admin/trips`) ✅ complete — group trips (v2)

Plan, track, split and share trip spending. **v2** turned a solo tracker into a group one: a trip now
has its own **split ledger** — `TripExpense` (the source of truth for every shared cost) + per-person
`TripExpenseShare` + `TripSettlement` (money handed between people) + `TripParticipant` (the people).
Syful's own cash/bank/wallet payments **also** post a linked `MoneyEntry` (so derived balances, the FX
seam, and the money manager keep working); everyone else's money never touches his ledger.

- **Posting rule** — a `TripExpense` posts a linked `MoneyEntry` DEBIT **only when** the payer is self **and** the funding account is a real spendable account (`CASH/BANK/MOBILE_WALLET/OTHER`, incl. the MYR wallet). **Never** for a `CREDIT_CARD` (deferred — card bills are settled in bulk separately, per Syful's rule) and **never** for a friend-paid expense. So a card/friend expense lives only in the trip ledger.
- **Splitting** — each expense is split among the selected participants: **equal** (default) or **exact** per-person amounts (largest-remainder cent distribution so shares always sum to the total). A participant's _spend_ = Σ their shares.
- **Who owes whom** — the report derives per-person `paid`/`spent`/`net` (BDT) and a **greedy minimal-transfer** settle-up list ("X pays Y ৳Z"). **Contributions / settle-ups** (`TripSettlement`, from→to) are pure trip-ledger — collected funds **never** post to the money accounts (a locked decision: group money stays out of personal net worth).
- **Participants** — per-trip names, each optionally linked to a `Beneficiary`. Every trip auto-seeds one `isSelf` "Me"; participants with split history are soft-deleted (`isActive`) so the math stays intact.
- **Foreign spending** uses a dedicated **MYR "trip wallet"** (`Trip.localWalletAccountId`), funded by a cross-currency `recordTransfer` (BDT→MYR, live rate prefilled). A self MYR-wallet expense posts + debits the wallet; leftover MYR = wallet balance. The funding TRANSFER is tagged `tripId` but **not** counted as spend.
- **Report** (`src/services/trips/report.ts`) — planned-vs-actual per category; **group cost** (all payers) vs **paid-by-me**; the **personal cash-flow split** (out-of-pocket cash/bank/wallet vs deferred credit-card, self only); spending by currency; per-day spend; per-person balances + who-owes-whom; and the wallet summary. Actuals valued in BDT via each row's **stored** rate; wallet leftover via the **live** rate.
- **Public shareable page** — `/trips/<slug>` (portfolio surface, Tailwind, no auth) renders an **aggregate-safe** cost guide (destination, dates, per-category + per-day totals in local + BDT, author intro). `getPublicTripSummary` never leaks participant names, per-person spend, who-owes-whom, or account fields; unpublished/unknown slug → 404.
- **Pages:** trips list (`TripsPage`) + per-trip detail (`[id]/TripDetailPage`: summary tiles, participants panel, budget vs actual, expenses table with payer/split/via, add-expense drawer with payer + `SplitEditor`, payments/contributions panel, who-owes-whom block, fund-wallet drawer, publish toggle). Orchestrator+hooks+components throughout.
- **Service-layer-first** (`src/services/trips/`): same functions back the API routes (`/api/admin/trips/*` incl. `/participants`, `/settlements`), the public page, and read-only **AI tools** (`list_trips`, `get_trip_report` with per-person balances, `list_trip_participants` — scope `/money`). Integration-tested in `src/services/trips/__tests__/trips.scenarios.test.ts` (self-cleaning). v1 trips migrate via `scripts/backfill-trip-groups.ts`.
- **Dynamic currencies:** currency selection is now sourced live from the FX feed (`getCurrencyOptions` → `GET /api/admin/currencies`) and offered via the searchable `CurrencySelect`/`CurrencyMultiSelect`, replacing the old hardcoded BDT/USD/EUR list across Money + Finance pickers.
...
### Booking — consultation scheduler (`/admin/bookings` + `/admin/settings/booking` + public form) ✅ functional

A **self-hosted, public-facing** appointment scheduler — visitors book a call from the portfolio home
page; the app creates a Google Calendar event with a Google Meet link and emails invites. No third-party
scheduler (Calendly etc.); the slot math and Google integration are implemented in-repo. **Read-write to
Google Calendar** (it reads free/busy and creates/deletes only the events it created).

- **Public flow** — the portfolio home page renders `<BookConsultation>` (`src/components/portfolio/`,
  a `<section id="booking">`) + a `<FloatingBookButton>`, both of which self-hide when booking isn't
  live. UX: duration chips → 14-day day grid → time-slot chips (lazily fetched per day) → visitor fields
  → **Cloudflare Turnstile** captcha → confirmed view with the Meet link + a self-cancel button. Backed by
  four **unauthenticated** routes: `GET /api/booking/config`, `GET /api/booking/slots`,
  `POST /api/booking/create`, `POST /api/booking/cancel`.
- **Anti-abuse** — a hidden honeypot field, per-IP rate limit (3/hour, 10/day), and server-side Turnstile
  verification (skipped only if `TURNSTILE_*` keys are unset).
- **Availability math** (`src/services/booking/slots.ts`) — candidate starts = working-hours ranges ×
  `slotIncrementMinutes`, filtered by `minNoticeHours`/`maxHorizonDays`, minus blackout days, minus DB
  bookings, minus Google free/busy, each padded by `bufferMinutes`. Timezone-safe (`Asia/Dhaka` default).
  `createBooking` inserts a `PENDING` row → creates the Calendar event → flips to `CONFIRMED`, rolling back
  to `CANCELLED` if Google fails.
- **Google integration** (`src/services/booking/googleCalendar.ts`) — plain-`fetch` OAuth + Calendar REST
  (no `googleapis` dep; mirrors `services/admin/googleDrive.ts`). **Reuses the same
  `GOOGLE_OAUTH_CLIENT_ID/SECRET`** as Drive backups — enable the Calendar API too and register the extra
  redirect `<AUTH_URL>/api/admin/booking/google/callback`. Refresh token AES-256-GCM encrypted
  (`AI_CONFIG_SECRET`) in the `BookingSettings` singleton.
- **Admin** — `/admin/bookings` (`BookingsPage`: filter + table + cancel) and `/admin/settings/booking`
  (`BookingSettingsPage`: master enable, Google connect/disconnect, slot rules, working-hours grid,
  blackout dates, WhatsApp button, meeting template). Session-guarded routes under `/api/admin/booking*`
  - `/api/admin/bookings`. Sidebar: **Bookings** (top-level) + **Settings → Booking**.
- **No AI tools / no slash command.** Tested in `src/services/booking/__tests__/booking.test.ts`
  (`npm run test:booking`, dev DB, self-cleaning).
- ⚠️ **Known debt:** the two admin pages are **not** decomposed into `hooks/`+`components/` like the rest
  of the admin (`BookingSettingsPage.tsx` ≈ 599 lines, over the 300-line orchestrator limit). Split before
  extending. See the audit note in `docs/PROJECT_STATUS_REPORT.md`.
...
| Date       | What was done                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-29 | **Documentation sync + health report** — full audit of docs/memory vs the codebase after a long feature run. Found the entire **Booking module** (shipped 2026-06-23) and **Trip Expense Manager** undocumented in PROJECT*PLANNING's schema table/folder tree, missing from memory, and absent from AI_TOOLS_REFERENCE. Added the Booking module section + 3 schema rows + folder-tree services (money/trips/booking/solar/solis, store/, hooks/, per-module types); added Money + Trip AI read-tool tables and corrected the tool footprint (66→**98**: 48 read + 50 write; per-scope /property 40, /finance 33, /money 19, /solar 9); documented `TURNSTILE*\*` env vars (`.env.example`+ CLAUDE.md) and the shared Google OAuth client (Drive + Calendar); rewrote the Trip memory to v2 and added a Booking memory. New`docs/PROJECT_STATUS_REPORT.md` captures the gap analysis + open items. No code changed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-07-27 | **Trip Expense Manager v2 — group trips** (`a8142e3`→`226d677`) — turned the solo tracker into a group one with its own split ledger. New models `TripParticipant`/`TripExpense`/`TripExpenseShare`/`TripSettlement` + `TripSplitMode` enum (migration `20260727000000_add_trip_groups`). Posting rule (self + real spendable account → linked `MoneyEntry` DEBIT; card/friend → trip-ledger only). Largest-remainder cent-exact split (EQUAL/EXACT), greedy minimal-transfer **who-owes-whom**, per-participant paid/spent/net, MYR trip wallet, aggregate-safe public `/trips/<slug>` page. Services `src/services/trips/*`, 13 API handlers, orchestrator+hooks+components admin UI, AI read tools `list_trips`/`get_trip_report`/`list_trip_participants` (scope `/money`), self-cleaning scenario tests. v1→v2 backfill `scripts/backfill-trip-groups.ts`. Then a full **security/optimization/standards audit** (`docs/TRIP_MANAGEMENT_AUDIT.md`, `3c9474a`) with findings #1–#7 fixed (`bb6466c`), #8 (CLOSED-trip freeze) deferred.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-07-26 | **Trip Expense Manager v1 + dynamic currencies** (`cf3f73b`) — first cut: `Trip`/`TripBudget` models, a trip as a tag over the money ledger, budget-vs-actual, MYR wallet funding, public share page. Same commit switched every currency picker from a hardcoded BDT/USD/EUR list to a **live FX-sourced** `CurrencySelect`/`CurrencyMultiSelect` (`getCurrencyOptions` → `GET /api/admin/currencies`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-06-23 | **Booking — public consultation scheduler** (`01f2e2c`→`112ce3e`) — a self-hosted appointment booker on the portfolio home page (`BookConsultation` + `FloatingBookButton`), Google Calendar + Meet, Cloudflare Turnstile + honeypot + per-IP rate limit. New `BookingSettings`/`BookingBlackout`/`Booking` models + `BookingStatus` enum; services `src/services/booking/*` (in-repo slot math + plain-`fetch` OAuth/Calendar REST reusing the Drive OAuth client); public routes `/api/booking/*` + admin `/api/admin/booking*`; admin UI at `/admin/bookings` + `/admin/settings/booking`; 30 self-cleaning tests (`npm run test:booking`). No AI tools. (Was undocumented until the 2026-07-29 sync above.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-07-11 | **Fixed Money dashboard income double-count** — the overview headline computed `income = recordedIncome + ventureTotal`, which double-counted every property/business earning that had been posted into the ledger via the opt-in `recordLinkedEntry` bridge (counted once as a ledger CREDIT, once as venture net). `useMoneyInsights` now treats the **ledger as the single source of truth**: Income = recorded ledger credits only, Savings = income − expense, and the monthly trend is ledger-only. Venture take-home (property net + business net) stays on its own read-only card as context, no longer summed into Income; the Income subtitle and the venture card tooltip/caption were reworded to say venture is context and is counted once it's posted to an account. No schema/service change — the service totals were already ledger-only; only the view-layer blend was removed. tsc + eslint + prettier clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
Every major module is now built (Property, Financial Tracker, Money Manager, Trip Expense Manager,
Booking, Solar, AI Assistant, Reports, Settings, Backups). Done since the old list: ~~sitemap/robots~~
(✅ `src/app/sitemap.ts` + `robots.ts`), ~~Property~~ ✅, ~~Finance~~ ✅, ~~AI tool use~~ ✅
(now read **and** write tools behind an approval gate). Remaining, in rough priority:

1. **Renovation Tracker** — the one admin module still a `StubPage` (`/admin/renovation`). Build line
   items from `House_Rebuilding_Construction.xlsx` (grand total ৳12,500,000), totals by category — mirror
   the Financial Tracker module pattern. No `src/services/renovation/` exists yet.
2. **Deploy to DigitalOcean** — droplet + Nginx + SSL + PM2 (see Deployment plan above); wire the
   `TURNSTILE_*` keys and the Booking Calendar OAuth redirect on the live host.
3. **Booking admin-page decomposition** (tech debt) — split `BookingSettingsPage.tsx` (~599 lines) and
   `BookingsPage.tsx` into `hooks/`+`components/` to match the rest of the admin (300-line limit).
4. **Trip audit item #8** — decide whether a `CLOSED` trip should freeze its ledger or still allow late
   corrections, then guard (or don't) in `expenses.ts`/`settlements.ts`. See `docs/TRIP_MANAGEMENT_AUDIT.md`.
5. **AI next frontier** — OpenAI + Gemini adapters (`services/ai/adapters/`); persist pending write-action
   cards/outcomes across sessions; tool **retrieval (tier 3)** if a single scope crosses ~80 tools
   (largest today is `/property` at 40). See `AI_TOOLS_REFERENCE.md` §8 and `AI_ENGINEERING_LEARNING_PROGRESS.md`.
6. **RAG / embeddings (Lesson 3.x)** — the next AI-engineering lesson, not yet started.

> Full current-state gap analysis and open-items list: **`docs/PROJECT_STATUS_REPORT.md`**.
