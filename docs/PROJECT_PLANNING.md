...
| `Trip`                                    | Trip Expense Manager — a trip as a tag over the money ledger; `destination`, `localCurrency` (e.g. MYR), `homeCurrency`, `startDate`/`endDate?`, `status` (PLANNING/ACTIVE/CLOSED), `localWalletAccountId?` (foreign wallet `MoneyAccount`), `publicSlug?`/`isPublic`/`publicIntro?` (shareable `/trips/<slug>` page). Totals derived from tagged `MoneyEntry` rows                                                                                                                                                                                                                                                                                                                           |
...
### Trip Expense Manager (`/admin/trips`) ✅ complete

Plan, track and share trip spending. A **trip is a tag over the money ledger** (`Trip` + nullable
`tripId`/`tripCategory` on `MoneyEntry`) — every trip expense is a normal DEBIT `MoneyEntry`, so it
reuses derived balances, the FX seam, and the credit-card account type. Nothing new is stored as a
running total.

- **Credit-card vs cash/bank** falls out for free: a card expense is a DEBIT on the `CREDIT_CARD` account (raises card debt only — deferred until you settle the bill via a bank→card transfer); a cash/bank expense debits that real account immediately (the "leisure balance" is just your existing accounts, per the design decision — no dedicated pot).
- **Foreign spending** uses a dedicated **MYR "trip wallet"** (`Trip.localWalletAccountId` → a local-currency `MoneyAccount`), funded by a cross-currency `recordTransfer` (BDT→MYR, live rate prefilled, editable). MYR spend debits the wallet; leftover MYR = wallet balance. The funding TRANSFER is tagged `tripId` but **not** counted as trip spend (avoids double-counting).
- **Fixed trip categories** (`TripCategory` enum). All trip DEBITs share one seeded **"Travel"** `MoneyCategory` in the ledger; `tripCategory` carries the fine-grained bucket, so trips stay one clean line in personal finance.
- **Report** (`src/services/trips/report.ts`) — planned-vs-actual per category, the **settlement split** (out-of-pocket cash/bank vs deferred credit-card), spending by currency, per-day spend, and the wallet summary (funded/spent/leftover). Actuals valued in BDT via each row's **stored** rate; the wallet leftover via the **live** rate.
- **Public shareable page** — `/trips/<slug>` (portfolio surface, Tailwind, no auth) renders an **aggregate-safe** cost guide (destination, dates, per-category + per-day totals in local + BDT, author intro). `getPublicTripSummary` never leaks account names/balances; unpublished/unknown slug → 404.
- **Pages:** trips list (`TripsPage`) + per-trip detail (`[id]/TripDetailPage`: summary tiles, budget vs actual, expenses table, add-expense + fund-wallet drawers, currency/day breakdown, publish toggle). Orchestrator+hooks+components throughout.
- **Service-layer-first** (`src/services/trips/`): same functions back the API routes (`/api/admin/trips/*`), the public page, and read-only **AI tools** (`list_trips`, `get_trip_report` — scope `/money`). Integration-tested in `src/services/trips/__tests__/trips.scenarios.test.ts` (self-cleaning).
