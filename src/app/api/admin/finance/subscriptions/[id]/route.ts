...
import { getSubscriptionDetail, updateSubscription, deleteSubscription } from "@/services/finance";
...
export const PUT = withApiError(
  async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const data = await updateSubscription(id, {
      ...body,
      ...(body.monthlyAmount != null && { monthlyAmount: Number(body.monthlyAmount) }),
    });
    return Response.json({ data });
  }
);
