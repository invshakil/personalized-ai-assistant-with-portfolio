...
import { getOneOffCharges, createOneOffCharge } from "@/services/property";
...
export async function GET(req: NextRequest) {
  ...
  const data = await getOneOffCharges({
...
export async function POST(req: NextRequest) {
  ...
  try {
    const data = await createOneOffCharge({ tenantId, label, amount, month, year, notes });
